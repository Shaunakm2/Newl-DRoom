...
// Admin approval picker — same modal/pattern as the public request picker,
// but for EXISTING pending bookings that already have an id. Each item can
// be resolved as: move to an alternate free room, approve anyway in the
// original (conflicting) room, or skip (leave it Pending for later).
function openApprovalConflictModal(items) {
...
function selectApprovalResolution(id, choice) {
    ...
    try {
      ...
      for (const it of s.items) {
        ...
        if (choice === 'anyway') {
          ...
          // choice is a room id — move the booking to that room, then approve
